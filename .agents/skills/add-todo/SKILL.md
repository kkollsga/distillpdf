...
`dev-docs/todos.md` is the *local* working index. It is **not** `ROADMAP.md`
(the committed, shared worklist at the repo root) — don't file committed roadmap
direction here, and don't touch `ROADMAP.md` from this skill.
...
- **Batch / deeper analysis** — a body of findings (a research report, an
  audit, a code review, a Sonnet fidelity review, inbox content) that contains
...
location is worth far more than a vague one. For a claimed extraction/layout
bug, confirm it's a real defect and not intended behaviour before filing it as a
bug (read the surrounding code/tests). Convert any relative date to an absolute
one.
...
- **Evidence** — reproduction / failing PDF / bench, if any.
- **Fix site + approach** — `file:line` + the shape of the change.
- **Test** — for correctness bugs, the unit/integration test (Rust
  `cargo test --lib` or `pytest tests/test_*.py`) the fix must land; for an
  extraction bug, the generated PDF fixture it needs (extend
  `tests/gen_fixtures.py` / `gen_tables.py` — self-generated, committable).
- **Effort** — rough size.
...
- Don't bump the version or touch code — this is backlog capture only.
