...
- **`mcp-servers/` is one externally-managed project, not a tree of repos.**
  Its subdirs (`code_review/`, `open_source/`, `kglite/`, …) are **not** notify
...
Filename: **`<YYYY-MM-DD>-from-distillpdf-<topic-slug>.md`** (date = session
...
(AGENTS.md "Route to the party who can act") — and clear the send bar below
first.
