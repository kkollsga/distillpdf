#!/usr/bin/env python3
"""Benchmark pylopdf (our wheel) on the same corpus, mirroring the others."""
...
for path in CORPUS:
    ...
    box = {}
    def run():
        d = pylopdf.Pdf.open(path); box["txt"] = d.extract_text(); box["pc"] = d.page_count()
    try:
        rec["t_text_s"] = round(timeit(run), 4)
        rec["pages"] = box["pc"]
        txt = box["txt"]; rec["chars"] = len(txt); rec["ok"] = True
        open(os.path.join(OUT, f"{name}.pylopdf.txt"), "w").write(txt)
...
json.dump(results, open(os.path.join(OUT, "results_pylopdf.json"), "w"), indent=2)
print("\nsaved -> out/results_pylopdf.json")
